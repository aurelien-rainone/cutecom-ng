...
#define PACKET_RX_TIMEOUT_SEC   (1000)
...
extern int32_t _getchar(unsigned short timeout); // msec timeout

// #define _putchar(c) do { printf("%d",c); } while(0) //write(c)
extern void _putchar(char c);

extern void _sleep(int miliseconds);
...
int32_t fymodem_send(uint8_t* txdata, size_t txsize, const char* filename)
{
  ...
  do {
    ...
    if (ch == ACK) {
      ch = _getchar(PACKET_RX_TIMEOUT_SEC);
      if (ch == CRC) {
        /* AR : addition to the original file, indicates that, from now on,
         * bytes transferred are the actual file data, and not for the
         * handshaking/synchronization part of the protocol
         * THIS SHOULD BE ADDED TO XMODEM AS WELL
         */
